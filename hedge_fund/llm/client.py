...
def make_llm(
    ...
    elif provider in {"Meituan", "MiniMax"}:
        # Both providers expose an OpenAI-compatible endpoint. MiniMax can use
        # its own credentials, or fall back to the shared Meituan gateway.
        from langchain_openai import ChatOpenAI
        gateway_key = api_key
        if provider == "MiniMax":
            gateway_key = os.getenv("MINIMAX_API_KEY") or os.getenv("MEITUAN_API_KEY")
        base_url = (
            os.getenv("MINIMAX_API_BASE") if provider == "MiniMax" else None
        ) or os.getenv(
            "MEITUAN_API_BASE", "https://aigc.sankuai.com/v1/openai/native"
        )
        chat = ChatOpenAI(
            model=model,
            api_key=gateway_key,
            timeout=timeout,
            max_retries=1,
            max_tokens=max(max_tokens, 8192) if provider == "MiniMax" else max_tokens,
            base_url=base_url,
        )
...
def _require_key(provider: str) -> str:
    ...
    if provider == "MiniMax":
        key = os.getenv("MINIMAX_API_KEY") or os.getenv("MEITUAN_API_KEY")
